...
func start_sync() -> void:
	...
	var dream: Landscape = load("res://src/dreams/landscapes/example_landscape.tscn").instantiate()
	dream.startup()
...
func update_sync(sync_ratio: float) -> void:
	...
	var interpolated := ours.interpolate_with(theirs, sync_ratio)
